...
static struct bt_ras_rreq m_rreq_pool[CONFIG_BT_RAS_RREQ_MAX_ACTIVE_CONN];
...
static void data_receive_finished(struct bt_ras_rreq *rreq)
{
	...
	rreq->on_demand_rd.cb(rreq->conn, rreq->on_demand_rd.counter_in_progress, error_code);
}
...
static int ack_ranging_data(struct bt_ras_rreq *rreq)
{
	...
	NET_BUF_SIMPLE_DEFINE(ack_ranging_data, RASCP_CMD_OPCODE_LEN + sizeof(uint16_t));
	net_buf_simple_add_u8(&ack_ranging_data, RASCP_OPCODE_ACK_RD);
	net_buf_simple_add_le16(&ack_ranging_data, rreq->on_demand_rd.counter_in_progress);

	err = bt_gatt_write_without_response(rreq->conn, rreq->cp.subscribe_params.value_handle,
					     ack_ranging_data.data, ack_ranging_data.len, false);
	if (err) {
...
void bt_ras_rreq_free(struct bt_conn *conn)
{
	...
	if (info.state == BT_CONN_STATE_CONNECTED) {
		...
		if (err != 0 && err != -EINVAL) {
			LOG_DBG("Failed to unsubscribe to RAS-CP: %d", err);
		}

		err = bt_gatt_unsubscribe(conn, &rreq->on_demand_rd.subscribe_params);
		if (err != 0 && err != -EINVAL) {
			LOG_DBG("Failed to unsubscribe to ondemand ranging data: %d", err);
		}

		err = bt_gatt_unsubscribe(conn, &rreq->rd_ready.subscribe_params);
		if (err != 0 && err != -EINVAL) {
			LOG_DBG("Failed to unsubscribe to ranging data ready: %d", err);
		}

		err = bt_gatt_unsubscribe(conn, &rreq->rd_overwritten.subscribe_params);
		if (err != 0 && err != -EINVAL) {
			LOG_DBG("Failed to unsubscribe to ranging data overwritten: %d", err);
		}
	}
